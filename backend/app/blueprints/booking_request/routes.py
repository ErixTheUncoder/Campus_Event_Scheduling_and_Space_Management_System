...
# student edit pending booking
@booking_requests_bp.patch("/<int:booking_id>")
def edit_booking(booking_id):
    data = request.get_json(silent=True) or {}
    resp, code = edit_booking_request(booking_id, data)
    return jsonify(resp), code

# student withdraw pending booking
@booking_requests_bp.patch("/<int:booking_id>/withdraw")
def withdraw_booking(booking_id):
    data = request.get_json(silent=True) or {}
    resp, code = withdraw_booking_request(booking_id, data)
    return jsonify(resp), code
